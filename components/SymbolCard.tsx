...
const SymbolCard: React.FC<SymbolCardProps> = ({ symbol, onClick, onEdit, onDelete }) => {
  return (
    <div
      ...
      className={`group relative flex flex-col items-center justify-center gap-0.5 text-center bg-surface-light dark:bg-surface-dark shadow-lg hover:shadow-primary/50 hover:scale-105 transition-all duration-200 cursor-pointer aspect-square p-1 sm:p-2 rounded-lg sm:rounded-xl border-2 border-transparent hover:border-primary-light`}
    >
      {symbol.imageUrl ? (
        <img src={symbol.imageUrl} alt={symbol.name} className="w-full h-full object-cover rounded-md" />
      ) : (
        <span className="text-3xl sm:text-4xl">{symbol.emoji}</span>
      )}
